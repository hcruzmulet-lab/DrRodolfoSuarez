  ...
  return (
    <nav
      ...
      style={{
        ...
        padding: "14px 40px", background: "rgba(var(--bg-rgb),.72)",
        backdropFilter: "blur(14px)", borderBottom: "1px solid rgba(var(--gold-rgb),.16)",
      }}
    >
      <a href="#top" style={{ display: "flex", alignItems: "center", gap: 13 }}>
        ...
        <span style={{ display: "flex", flexDirection: "column", lineHeight: 1.1 }}>
          <span style={{ fontFamily: "var(--font-serif), 'Cormorant Garamond', serif", fontSize: 19, fontWeight: 600, letterSpacing: ".3px", color: "var(--text-strong)" }}>{site.doctor}</span>
          <span style={{ fontSize: 10.5, letterSpacing: "2.4px", textTransform: "uppercase", color: "var(--gold)" }}>{site.specialty}</span>
        </span>
      </a>
      <div data-nav-links style={{ display: "flex", alignItems: "center", gap: 34 }}>
        {nav.map((n) => (
          <a key={n.href} href={n.href} style={{ fontSize: 13.5, fontWeight: 500, color: "rgba(var(--text-rgb),.8)" }}>{n.label}</a>
        ))}
      </div>
